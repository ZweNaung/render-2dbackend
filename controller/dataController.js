...
const dailyDataInsertController = async (req, res, fixedTime) => {
    try {
        ...
        const dateCheck = await historyForTwoDModel.findOne({ date });
        ...
        if (dateCheck) {
            ...
            await historyForTwoDModel.updateOne({ date }, { $push: { child: { $each: child } } });

        } else {

            const data = new historyForTwoDModel({ date, child });
            await data.save();
            ...
            if (totalCount > 90) {
                ...
                const oldDocs = await historyForTwoDModel.find()
                    .sort({ _id: 1 })
                    .limit(deleteCount);
...
const handleGetDailyData = async (req, res) => {
    try {
        ...
        const dailyResult = await historyForTwoDModel.findOne({ date });
        if (!dailyResult) {
...
const getAllHistory = async (req, res) => {
    try {
        const allHistory = await historyForTwoDModel.find();
